...
function ready(){
  ...
  textarea.id = "jvTextarea";
  textarea.className = "form-control";
  textarea.setAttribute("rows",5);

  textarea.addEventListener(
             'blur',function () {
                var textVal = document.getElementById("jvTextarea").value;
                var html = jsonObjToHTML(JSON.parse(textVal));//eval("("+textVal+")"));
                document.getElementById("jvContent").innerHTML = html;
              },false );
  document.body.appendChild(textarea) ;
  jfContent = document.createElement("div");
  //jfContent.innerHTML = html;
  jfContent.id = 'jvContent' ;
